...
class Screen {
    ...
    public static void printBoard(Board bor) {
        ...
        public static void printChessFunc(Board bor, Func<int,int,bool> del,ConsoleColor color=ConsoleColor.Red,int delay=0 ){
        Console.Clear();
            Console.Clear();
            for (int i = 0; i < bor.lines; i++) {
                Console.Write(8 - i + " ");
                for (int j = 0; j < bor.columns; j++) {
                    bool funcresult=del.Invoke(i,j);
                    Piece piece = bor.piece(i,j);
                    if(funcresult&&piece!=null){
                        ConsoleColor CurrentConsoleColor=Console.BackgroundColor;
                        Console.BackgroundColor=color;
                        Screen.printPiece(bor.piece(i, j));
                        Console.Write(" ");
                        Console.BackgroundColor=CurrentConsoleColor;    
                    }
                    else if(funcresult){
                        ConsoleColor CurrentConsoleColor=Console.BackgroundColor;
                        Console.BackgroundColor=color;
